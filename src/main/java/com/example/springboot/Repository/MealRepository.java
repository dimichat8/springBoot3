...
public interface MealRepository extends JpaRepository<Meal, Long> {
    ...
    @Query("SELECT c.customer_id, m.mealPlan, c.firstName, c.lastName, m.dateFrom, m.dateTo FROM Meal m JOIN m.customer c WHERE c.customer_id = :customerId and m.dateFrom = :dateFrom and m.dateTo = :dateTo")
    List<Meal> getAllMealDataAccordingToDates(@Param("customerId") Long customerId, @Param("dateFrom") LocalDate dateFrom, @Param("dateTo") LocalDate dateTo);
